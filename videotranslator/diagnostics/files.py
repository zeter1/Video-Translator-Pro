...
- У warning/error в `diagnostic` есть стабильные `event_code`, `family`, `pipeline_stage`, `failure_kind`, `impact`, `recovery_action` и `signature`.
- Для перевода поле `response_validation` объясняет, почему строка провайдера признана Error/HTML/block page и была заблокирована до checkpoint/TTS.
- При исправлении начинайте с первой высокоприоритетной `issue_card`, затем ищите указанные `code_search`; не читайте весь проект без необходимости.
